...
product film is live. The real production microphone-capture lifecycle is proven
on a device; in-app readback remains unproven.
The filmed MCP run stopped before signing, submission, broadcast, or
...
| Hybrid voice candidate | **CAPTURE LIFECYCLE LIVE-PROVEN; READBACK PENDING — Supabase spend reservations are applied and sensitive provider variables are configured; a bodyless authenticated `POST /api/voice/token` returns `200` with a real provider session and durable quota headers while a declared request body is still refused with `413`; on 2026-08-14 a real device drove permission grant → live `429 VOICE_CONCURRENCY_LIMITED` → `200` mint → `LISTENING` → clean release; no dictation was spoken, so transcript delivery and in-app ElevenLabs readback stay pending** |
