...
| `E10` | SOURCE + TEST PROVEN; MIGRATION/DEPLOY PENDING | The first-party workspace runs exactly four ordered review stages and unlocks Freeze only for the current HMAC-attested input and durable receipt UUID. Upstream edits invalidate the review. Audit memory is bounded, expiring, user-deletable, and cannot alter policy/code. | `apps/web/lib/server/agentControl.ts`, `apps/web/components/AgentReviewLauncher.tsx`, `apps/web/lib/reviewGate.ts`, `apps/web/test/agentControl.test.ts`, `apps/web/test/reviewGate.test.ts`, `supabase/migrations/20260811052236_settlement_agent_control_plane.sql` | `SOURCE/TEST PROVEN · HOSTED MIGRATION PENDING`; never “self-evolving” |
| `E11` | SOURCE IMPLEMENTED; EXTERNAL CONFIG PENDING | Supabase is canonical auth/RLS identity; separate sign-in/create-account, strict callback, and branded return page exist. Privy is a linked-wallet identity bridge only and fails closed until dashboard/JWKS/domain/identity-token/app-ID/verifier setup. Branded inbound email needs verified-domain SMTP or Send Email Hook. | `apps/web/app/auth/`, `apps/web/app/api/privy/session/route.ts`, `apps/web/lib/privy/`, `docs/integrations/privy.md` | show Supabase auth only after live probe; show Privy identity only after its external configuration passes |
...
| 3 | real account return, durable tab, editable complex receipt, caller-entered participants, cent-perfect allocation | `E04`, `E05`, `E11` | `G-PRODUCT-FINAL`, `G-AUTH-LIVE`; Privy identity separately gated |
